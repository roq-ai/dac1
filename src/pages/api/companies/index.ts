...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  async function createCompany() {
    ...
    const body = { ...req.body };
    if (body?.greeting_template?.length > 0) {
      const create_greeting_template = body.greeting_template;
      body.greeting_template = {
        create: create_greeting_template,
      };
    } else {
      delete body.greeting_template;
    }
